...
def run(
    ...
    project=YOLO_ROOT / "runs/val",  # save to project/name
    ...
    Args:
        ...
        project (str | Path, optional): Directory to save results. Default is YOLO_ROOT/'runs/val'.
    ...
    if dataloader is None:
        ...
        if model_mode: # TODO
            LOGGER.error("[val] DetectionModel has not 'stride' attribute (Runtime Rooot)")
            sys.exit(-1)
    ...
    if hasattr(model, "names"):
        ...
        names = model.module.names  # get class names
    ...
    for batch_i, (im, targets, paths, shapes) in enumerate(pbar):
        ...
        with dt[1]:
            if compute_loss:
                ...
                preds, train_out = (model(im, augment=augment), None)

        # Loss
        if compute_loss:
            loss += compute_loss([torch.dequantize(t) for t in train_out], targets)[1]  # box, obj, cls
...
def parse_opt():
    ...
    parser.add_argument("--project", default=YOLO_ROOT / "runs/val", help="save to project/name")
