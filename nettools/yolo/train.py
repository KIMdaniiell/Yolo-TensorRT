...
import models
YOLO_ROOT = Path(models.__file__).resolve().parents[1]
...
from .quantization.quantizable_model_ema import QuantizableModelEMA
from ..general import has_qconfig
...
def train(hyp, opt, device, callbacks):
    ...
    if not evolve:
        ...
        ### Object items spoofing to avoid some YAML parsing errors ###
        ...
        for object_arg_name in object_args.keys():
            object_arg = getattr(opt, object_arg_name)
            if not object_arg is None:
                setattr(opt, object_arg_name, str(type(object_arg)))
                object_args[object_arg_name] = object_arg
        yaml_save(save_dir / "opt.yaml", vars(opt))
        for object_arg_name in object_args.keys():
            setattr(opt, object_arg_name, object_args[object_arg_name])
        del(object_args)
    ...
    with torch_distributed_zero_first(LOCAL_RANK):
        data = yaml_load("datasets/coco.yaml")
        path = Path(data["path"])
    ...
    pretrained = weights.endswith(".pt") or (not opt.model is None)
    quantizable = False
    if pretrained:
        with torch_distributed_zero_first(LOCAL_RANK):
        ...
        if not opt.model is None:
            model = opt.model
            quantizable = has_qconfig(model)
        else:
    ...
    # EMA
    ema = QuantizableModelEMA(model) if RANK in {-1, 0} else None
    ...
    # Trainloader
    if not opt.train_dataloader is None:
        train_loader = opt.train_dataloader
        dataset = opt.dataset
        print("[train_quant] Пропуск 'create_dataloader()' (использование существующего объекта train_dataloader)")
    else:
    ...
    # Process 0
    if RANK in {-1, 0}:
        if not opt.val_dataloader is None:
            val_loader = opt.val_dataloader
            print("[train_quant] Пропуск 'create_dataloader()' (использование существующего объекта val_dataloader)")
        else:
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        if RANK in {-1, 0}:
            ...
            if not noval or final_epoch:  # Calculate mAP
                ...
                if quantizable:
                    if device.type != "cpu":
                            LOGGER.info("Quantized inference is not supported on CUDA. Moving quantized model to CPU for final validation")

                    _model = deepcopy(de_parallel(model))
                    _model.to('cpu')
                    _model.eval()
                    _compute_loss  = ComputeLoss(_model)
                    torch.quantization.convert(_model, inplace=True)
                else:
                    _model = ema.ema
                ...
                results, maps, _ = validate.run(
                    ...
                    plots=False,
                    callbacks=callbacks,
                    compute_loss=_compute_loss if quantizable and device.type else compute_loss,

                    iou_thres=0.65 if is_coco else 0.60,
                    conf_thres=0.001 if not quantizable else 0.25,
            ...
            if (not nosave) or (final_epoch and not evolve):  # if save
                ckpt = {
                    ...
                    "model": deepcopy(de_parallel(model)).half() if not quantizable else None,
                    "ema": deepcopy(ema.ema).half() if not quantizable else None,
                ...
                ### Object items spoofing to avoid some saving errors ###
    ...
    if RANK in {-1, 0}:
        ...
        for f in last, best:
            if f.exists():
                if not quantizable:
                    strip_optimizer(f)  # strip optimizers # TODO 
                if f is best:
                    LOGGER.info(f"\nValidating {f}...")
                    ...
                    if quantizable:
                        ...
                        _model.load_state_dict(
                            torch_load(f, map_location=torch.device("cpu"))["state_dict"])
                        _compute_loss  = ComputeLoss(_model)
                        torch.quantization.convert(_model, inplace=True)
                    else:
                        _model = attempt_load(f, device).half()
                    ...
                    results, _, _ = validate.run(
                        ...
                        # iou_thres=0.65 if is_coco else 0.60 if not quantizable else 0.45,  # best pycocotools at iou 0.65
                        ...
                        half=False, 
                        dataloader=val_loader, 
                        verbose=False)  # val best model with plots
                    if is_coco:
...
def parse_opt(known=False):
    ...
    parser.add_argument("--weights", type=str, default=YOLO_ROOT / "yolov5s.pt", help="initial weights path")
    parser.add_argument("--cfg", type=str, default="", help="model.yaml path")
    parser.add_argument("--data", type=str, default=YOLO_ROOT / "data/coco128.yaml", help="dataset.yaml path")
    parser.add_argument("--hyp", type=str, default=YOLO_ROOT / "data/hyps/hyp.scratch-low.yaml", help="hyperparameters path")
    ...
    parser.add_argument(
        "--evolve_population", type=str, default=YOLO_ROOT / "data/hyps", help="location for loading population"
    )
    ...
    parser.add_argument("--project", default=YOLO_ROOT / "runs/train", help="save to project/name")
    ...
    # Renamed to `ignore-layers`. Argument names matching (ipykernel_launcher -f) caused interpretation problem
    # parser.add_argument("--freeze", nargs="+", type=int, default=[0], help="Freeze layers: backbone=10, first3=0 1 2")
    parser.add_argument("--ignore-layers", nargs="+", type=int, default=[0], help="Ignore layers: backbone=10, first3=0 1 2")
    ...
    ### Modded for torch.ao.quantization support
    parser.add_argument("--model", required=False, default=None, help="Preinitialized & quantizaed Torch nn model")
    parser.add_argument("--train-dataloader", required=False, default=None, help="Torch dataloader for training")
    parser.add_argument("--val-dataloader", required=False, default=None, help="Torch dataloader for validation")
    parser.add_argument("--dataset", required=False, default=None, help="Dataset")
...
def main(opt, callbacks=None):
    ...
    if RANK in {-1, 0}:
        ...
        check_git_status()
        check_requirements(YOLO_ROOT / "requirements.txt")
    ...
    if opt.resume and not check_comet_resume(opt) and not opt.evolve:
        ...
        assert len(opt.cfg) or len(opt.weights) or (not opt.model is None), "either --cfg or --weights or --model must be specified"
        print(f"[train_quant] {'MODEL' if (not opt.model is None) else 'WEIGHT'} MODE")

        if opt.evolve:
            if opt.project == str(YOLO_ROOT / "runs/train"):  # if default project name, rename to runs/evolve
                opt.project = str(YOLO_ROOT / "runs/evolve")
    ...
    print(f"[train_quant] Device = {str(device).upper()}")
    ...
    # Evolve hyperparameters (optional)
        ...
        if opt.resume_evolve is not None:
            assert os.path.isfile(YOLO_ROOT / opt.resume_evolve), "evolve population path is wrong!"
            with open(YOLO_ROOT / opt.resume_evolve, errors="ignore") as f:
...
def run(**kwargs):
    ...
    Args:
        weights (str, optional): Path to initial weights. Defaults to YOLO_ROOT / 'yolov5s.pt'.
        cfg (str, optional): Path to model YAML configuration. Defaults to an empty string.
        data (str, optional): Path to dataset YAML configuration. Defaults to YOLO_ROOT / 'data/coco128.yaml'.
        hyp (str, optional): Path to hyperparameters YAML configuration. Defaults to YOLO_ROOT / 'data/hyps/hyp.scratch-low.yaml'.
        ...
        evolve (int, optional): Evolve hyperparameters for a specified number of generations. Use 300 if provided without a
            value.
        evolve_population (str, optional): Directory for loading population during evolution. Defaults to YOLO_ROOT / 'data/ hyps'.
        ...
        project (str, optional): Directory for saving training runs. Defaults to YOLO_ROOT / 'runs/train'.
        ...
        ignore_layers (list, optional): Layers to ignore, e.g., backbone=10, first 3 layers = [0, 1, 2]. Defaults to [0].
        ...
        # --------------------------------- #
        model (any, optional): Preinitialized and quantizaed Torch model
        train_dataloader (any, optional) | Torch dataloader for training
        val_dataloader (any, optional) | Torch dataloader for validation
        dataset (any, optional) | Dataset
